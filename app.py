...
async def _futures_ready_check():
    try:
        res = await _fapi("GET", "/fapi/v2/balance", {})
        log.info("futures_ready_ok", assets=len(res))
        return True
    except Exception as e:
        msg = f"Futures API not usable: {e}"
        log.error("futures_ready_fail", error=str(e))
        try:
            await alert(f"❗️<b>LIVE ORDER DISABLED</b>\n{msg}\nCheck: API key has <b>Futures enabled</b>, no IP block, and USDT-M account is active.", "SYSTEM")
        except: pass
        return False

# --- Exchange filters (tick/step/minQty) ---
...
async def live_force_execute(sym: str, side: str, entry: float, sl: float) -> Optional[Dict[str,Any]]:
    ...
    if not api_key or not api_secret:
        log.warning("force_no_keys")
        await alert(f"❗️<b>NO API KEYS</b> — cannot place live order for {sym}\nAdd BINANCE_API_KEY / BINANCE_API_SECRET.", sym)
        return None

    try:
        if not _SYMBOL_FILTERS:
        ...
        if qty <= 0 or math.isinf(qty) or math.isnan(qty):
            m = f"Qty invalid for {sym}: qty={qty}, entry={entry}, sl={sl}, equity={equity}"
            log.warning("force_qty_invalid", msg=m)
        ...
        try:
            resp = await _fapi("POST","/fapi/v1/order", body)
            log.info("force_market_ok", symbol=sym, resp=resp)
            oid = resp.get("orderId")
            await alert(f"✅ <b>LIVE ENTRY</b> {sym} ({side})\nQty: {qty:.6f}  @ ~{px:.8f}\norderId: {oid}\nDRY_RUN: false | FORCE: true", sym)
        except Exception as e:
            em = str(e)
            log.error("force_market_fail", symbol=sym, error=em)
            await alert(
                f"❌ <b>ORDER ERROR</b> {sym} ({side})\n{em}\n"
                f"Qty: {qty:.8f}  minQty≈{min_qty}  Price≈{px:.8f}\n"
                f"Equity:{equity}  Risk%:{max_risk_pct}  NotionalCap%:{notional_max_pct} Lev:{leverage}",
                sym
            )
            return None
        ...
        try:
            ...
            await alert(f"⚠️ SL place failed for {sym}: {e}", sym)

        return resp

    except Exception as e:
        em = str(e)
        log.error("force_exec_crash", symbol=sym, error=em)
        await alert(f"❌ <b>LIVE EXEC CRASH</b> {sym}\n{em}", sym)
        return None
